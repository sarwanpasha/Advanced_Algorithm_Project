...
start_time = time.time()
#https://www.kaggle.com/sepidafs/text-classification-with-glove-lstm-gru-99-acc
aaa = (X_train.shape)
MAX_NB_WORDS = aaa[1]
EMBEDDING_DIM = 500
Memory_Units = 200
...
#model.fit(np.array(X_train), np.array(y_train), epochs=epochs, batch_size=batch_size,validation_split=0.1,callbacks=[EarlyStopping(monitor='val_loss', patience=3, min_delta=0.0001)])
#y_pred_2 = model.predict(X_test)
#y_pred = model.predict_classes(X_test, verbose=0)
#end_time = time.time() - start_time
#print("LSTM Total Time in seconds =>",end_time)
...
#gru_model.add(Embedding(MAX_NB_WORDS, emb_dim, trainable = False, weights=[X]))
gru_model.add(Embedding(MAX_NB_WORDS, EMBEDDING_DIM, input_length=np.array(X).shape[1]))
gru_model.add(GRU(Memory_Units, return_sequences=False))
gru_model.add(Dropout(0.2))
gru_model.add(Dense(max(unique_labels)+1, activation = 'softmax'))
gru_model.compile(loss='sparse_categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
print(gru_model.summary())


gru_model.fit(np.array(X_train), np.array(y_train), epochs=epochs, batch_size=batch_size,validation_split=0.1,callbacks=[EarlyStopping(monitor='val_loss', patience=3, min_delta=0.0001)])
y_pred_2 = gru_model.predict(X_test)
y_pred = gru_model.predict_classes(X_test, verbose=0)
end_time = time.time() - start_time
print("GRU Total Time in seconds =>",end_time)
# reduce to 1d array
y_pred_2 = y_pred_2.flatten()
y_pred = y_pred.flatten()
print(y_pred[0:10])
...
RC_acc = accr[1] #metrics.accuracy_score(y_test, y_pred)
